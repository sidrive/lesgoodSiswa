    ...
    mAuth.signInWithCredential(credential).addOnCompleteListener(
        task -> {
          ...
          if (task.isSuccessful()){
            ...
            Log.e("firebaseAuthWithGoogle", "LoginPresenter" + task.getException());
            activity.showLoading(false);
            ...
            firebaseUserService.getAuthWithGoogle(activity, acct)
                    .addOnCompleteListener(activity, task -> {
                        if (task.isSuccessful()) {
                            ...
                            activity.showLoading(false);
                            activity.showLoginFail("Gagal Masuk");
                          Log.e("getAuthWithGoogle", "LoginPresenter 2" + task.getException().toString());
                        }
                    });
        } else {
            activity.showLoginFail("Result is Failed : Gagal Masuk");
          Log.e("getAuthWithGoogle", "LoginPresenter 1" + result.getStatus());
        }
    }
